...
echo "==============================================="
echo "  Desinstalando Office 365 WineCX (Fedora family)"
echo "==============================================="
...
rm -f "$DESCARGAS/MSO365.zip" "$DESCARGAS/MSO365" "$DESCARGAS/winecx.deb" \
      ...
      "$DESCARGAS/winecx-fedora.zip"

# Cache descarga
rm -rf "$CACHE_DIR"

# Winetricks instalado por nosotros
[ -f /usr/local/bin/winetricks ] && sudo rm -f /usr/local/bin/winetricks
...
echo
echo "Paquetes dnf instalados como dependencia se conservan."
echo "Para removerlos manualmente (revisar antes):"
echo "  sudo dnf remove cabextract msitools samba-winbind msttcore-fonts-installer"
